...
# Sistema de Gestión — Licores

> App Flutter (Android + Windows) con backend Supabase.  
> Un solo codebase, un solo usuario, siempre conectado.
...
|---|---|---|
|1|Inventario|CRUD de productos, categorías, alertas de stock mínimo|
|2|POS / Ventas|Carrito, registro de venta, descuento automático de stock|
|3|Mayoristas|Clientes, pedidos, cuentas por cobrar, pagos parciales|
|4|Contabilidad|Dashboard de ingresos, utilidad bruta, productos top|
|5|Gastos personales|Registro diario de egresos, resumen mensual|

---

## Script SQL — Supabase

Recomendado: guardar este SQL como una migración versionada, por ejemplo
`supabase/migrations/202605190001_schema_inicial.sql`, y aplicarla con
`supabase db push`. Para una instalación rápida también puede ejecutarse en el
**SQL Editor** de Supabase en el orden presentado.
...
**Android:** `flutter build apk --release`. Instalar directo en el dispositivo (no requiere Play Store para uso personal).

**Windows:** `flutter build windows --release`. El ejecutable queda en `build/windows/x64/runner/Release/`. Empaquetar con Inno Setup o simplemente copiar la carpeta.

**Supabase:** En producción, mantener RLS activo, no crear políticas para el rol `anon` y revisar que el rol `authenticated` solo sea usado por el dueño del negocio. La `anon key` puede existir en el cliente, pero la protección real debe quedar en RLS y en las políticas.

**Migraciones:** versionar cada cambio de esquema en `supabase/migrations/` y aplicarlo con `supabase db push`. Evitar cambios manuales sin registrar porque después son difíciles de reproducir en otro equipo o proyecto.

**Backup:** Supabase incluye backups automáticos diarios en el plan free. Para reducir pérdida de datos, programar exportaciones periódicas con Supabase CLI (`supabase db dump`) o `pg_dump` y guardar copias fuera de Supabase.

---

_Generado para el sistema de gestión de licores — Tunja, Colombia._
