...
- Supplier rebates are **not** a separate balance. A short-lived `rebates_proveedor` module was added and then removed (see `202609050000_revertir_rebates.sql`): in practice the supplier discounts the rebate off the order's invoice rather than handing over a credit to spend later. It is recorded as a **negative `ajuste`** on the purchase, which lowers the total so exactly the cash that really left is subtracted. `metodo_pago` still carries an unused `'rebate'` value — Postgres cannot drop enum values — but nothing reads it.
- Purchase invoices are editable via the `editar_compra` RPC (reverses the old lines and applies the new ones in one transaction, keeping the compra id so its `pagos_compras` stay attached). It recomputes `valor_deuda` as new total minus payments already made, and refuses to drop a purchase out of `credito` while live payments exist. Known limitation, shared with `anular_compra`: reversing lines returns stock but does not unwind the weighted-average cost they contributed.
